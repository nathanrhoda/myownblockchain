...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((currentTime - msgTime) < interval) {
    ...
    getBlockByHeight(height) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.fimd(p => p.height === height);
            if(block){
                resolve(block);
            } else {
                resolve(null);
    ...
    getStarsByWalletAddress (address) {
        ...
        let stars = [];
        return new Promise((resolve, reject) => {
            let blockArray = self.chain.filter(p => p.address === address);
            ...
            for(let block of self.chain) {
                if(!(block.height === 0)){
                    ...
                    if(!(previousBlock.hash === currentBlock.previousBlockHash)) {
                        errorLog.push(`Hash ${currentBlock.hash} located at Height ${currentBlock.height} has been tampered with.`);
                    }
                }                   
            }
