...
class Block {
    ...
    validate() {
        ...
        return new Promise((resolve, reject) => {
            ...
            //Set to null to calculate hash of object without hash set
            self.hash = null;            

            // Recalculate the hash of the Block            
            let calculatedHash = SHA256(JSON.stringify(self)).toString();    
            
            // Re assign hash value to original 
            self.hash = hash;
    ...
    getBData() {
        ...
        return new Promise((resolve, reject) =>{
            ...
            //let decodedData = new Buffer(hexData, 'hex');

            // Parse the data to an object to be retrieve.
            let jsonData = JSON.parse(decodedData);

            // Resolve with the data if the object isn't the Genesis block
            if(!(jsonData === 'Genesis Block' && self.height === 0)) {
                resolve(jsonData);                
            } else {
